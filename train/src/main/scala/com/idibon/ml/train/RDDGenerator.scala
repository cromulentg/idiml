...
/** RDDGenerator
  ...
  * */
class RDDGenerator (sc: SparkContext, filename: String, pipeline: FeaturePipeline) {
  ...
  /** Produces an RDD of LabeledPoints for each distinct label name.
    ...
    */
  def getLabeledPointRDDs : HashMap[String, RDD[LabeledPoint]] = {
    ...
    val labelLPs = HashMap[String, ListBuffer[LabeledPoint]]()
    for (line <- Source.fromFile(filename).getLines()) {
      ...
      if (!labelLPs.contains(label)) {
        labelLPs(label) = new ListBuffer[LabeledPoint]()
      }
      ...
      labelLPs(label) += LabeledPoint(labelNumeric, featureVector)
    }

    val labelRDDs = HashMap[String, RDD[LabeledPoint]]()
    for ((label, lp) <- labelLPs) {
      labelRDDs(label) = sc.parallelize(lp)
    }
