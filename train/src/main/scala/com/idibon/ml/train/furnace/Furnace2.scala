...
trait Furnace2[+T <: PredictResult] {
  ...
  /** Trains the model synchronously
    ...
    */
  protected def doHeat(options: TrainOptions): PredictModel[T]
  ...
  /** Trains the model asynchronously
    ...
    */
  def heat(options: TrainOptions): Future[PredictModel[T]] = {
    implicit val executor = ExecutionContext.global
    val model = Promise[PredictModel[T]]()
    Future {
      model.complete(Try(doHeat(options)))
    }
    model.future
  }
}
...
object Furnace2 {
  ...
  type FurnaceBuilder = (Engine, String, JObject) => Furnace2[_]

  // master registry of all furnaces
  private[this] val _registry = mutable.Map[Type, mutable.Map[String, FurnaceBuilder]]()
  ...
  /** Register a new furnace type
    ...
    */
  private[train] def register[T <: PredictResult: TypeTag](tpe: String, b: FurnaceBuilder) {
    val r = _registry.getOrElseUpdate(typeOf[T], mutable.Map[String, FurnaceBuilder]())
    r += (tpe -> b)
  }
...
trait HasFurnaces[T <: PredictResult] {
  ...
  /** Trains all of the furnaces within this container in sequence
    ...
    */
  protected def heatFurnaces(options: TrainOptions): Seq[PredictModel[T]] = {
    ...
    furnaces.map(furnace =>
      Await.ready(furnace.heat(options), options.maxTrainTime).value match {
