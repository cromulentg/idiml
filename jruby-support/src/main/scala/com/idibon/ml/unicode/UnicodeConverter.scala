...
object UnicodeConverter {
  ...
  /**
    * Converts a document span represented by length and offset in UTF-32 (code points)
    ...
    */
  def from32to16(content: String, offset32: Int, length32: Int): (Int, Int) = {
    /* Given the code points of this content, iterate through
     * them and sum the Character.charCount of each point.
     * In UTF-16 this will be 1 for bmp, and 2 for non-bmp, as opposed
     * to 1 for all planes.
     */
  ...
  /**
    * Convert document spans represented by offset, length tuples from UTF-16 relative indexes to
    * UTF-32. Primarily for the purpose of converting span predictions generated by IdiML to UTF-32
    * based environments.
    *
    * @param content the document content
    * @param offset16 the offset the span starts at in this document (UTF-16)
    * @param length16 the length of this span (UTF-16)
    * @return a tuple of offset, length representing the same span in UTF-32 (code points)
    */
  def from16to32(content: String, offset16: Int, length16: Int): (Int, Int) = {
