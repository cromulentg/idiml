...
class ClassificationSpec extends FunSpec with Matchers {
  ...
  describe("#average") {
    ...
    it("should return the superset of flags and significant features") {
      val result = Classification.average(Seq(
        Classification("foo", 0.5f, 1, 0x5a5a, Seq("bar" -> 0.5f)),
        Classification("foo", 1.0f, 1, 0xa5a5, Seq("baz" -> 0.5f))), (c) => c.matchCount)
      result.label shouldBe "foo"
      result.probability shouldBe 0.75f
      result.matchCount shouldBe 2
      result.flags shouldBe 0xffff
      result.significantFeatures shouldBe Seq("bar" -> 0.5f, "baz" -> 0.5f)
    }
  }
  ...
  describe("#reduce") {
    it("should only consider FORCED values if present") {
      val result = Classification.reduce(Seq(
        Classification("foo", 1.0f, 2, PredictResultFlag.mask(PredictResultFlag.FORCED), Seq("bar" -> 1.0f)),
        Classification("foo", 0.0f, 6, PredictResultFlag.mask(PredictResultFlag.FORCED), Seq("baz" -> 0.0f)),
        Classification("foo", 0.99f, 2, 0, Seq("hello" -> 0.9f, "world" -> 0.9f))))
      result.label shouldBe "foo"
      ...
      result.significantFeatures shouldBe Seq("bar" -> 1.0f, "baz" -> 0.0f)
    }
  }
}
